...
/** 按渠道自动更新检测
 ...
 如果您想自己控制自动更新操作流程，请实现AVAnalyticsDelegate的appUpdate方法。
...
/** 设置AVAnalytics代理,已经startWithAppkey:所取代,不要再使用，原有代码可以删除
 */
...
/** 友盟模块启动
 [AVAnalytics startWithAppkey:]通常在application:didFinishLaunchingWithOptions:里被调用监听
 App启动和退出事件，如果你没法在application:didFinishLaunchingWithOptions:里添加友盟的[AVAnalytics startWithAppkey:]
 方法，App的启动事件可能会无法监听，此时你就可以手动调用[AVAnalytics startSession:nil]来启动友盟的session。
